...
type Reject = (errorData: ErrorData) => void
...
function checkJavaRuntime(): Promise<string> {
    return new Promise((resolve, reject) => {
        ...
        findJavaHome((err: any, home: string) => {
            if (err) {
...
function checkJavaVersion(javaHome: string): Promise<number> {
    return new Promise((resolve, reject) => {
        cp.execFile(javaHome + '/bin/java', ['-version'], {}, (_error, _stdout, stderr) => {
...
function openJDKDownload(reject: Reject, cause: string) {
...
function invalidJavaHome(reject: Reject, cause: string) {
